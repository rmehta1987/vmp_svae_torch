...
def init_recognition_params(theta, nb_components, seed=0, param_device='cuda'):
    ...
    pi_k_init = torch.nn.softmax(torch.randn((nb_components,)))

    pi_k = init_tensor_gpu_grad(pi_k_init, trainable=True, device=param_device)
...
def inference(y, phi_gmm, encoder_layers, decoder_layers, nb_samples=10, stddev_init_nn=0.01, seed=0):

    # Use VAE encoder
    x_given_y_phi_model = Encoder(layerspecs=encoder_layers)
    x_given_y_phi = x_given_y_phi_model.forward(y)
    ...
    y_reconstruction_model = Decoder(layerspecs=decoder_layers)
    y_reconstruction = y_reconstruction_model.forward(x_k_samples)
...
def e_step(phi_enc, phi_gmm, nb_samples, seed=0):
    """
...
def compute_log_z_given_y(eta1_phi1, eta2_phi1, eta1_phi2, eta2_phi2, pi_phi2):
    ...
    w_eta1 = tf.einsum('nuj,nkuv->nkj',eta2_phi1,mu_eta2_1_eta2_2) # Shape: NxKxL
...
def compute_elbo(y, reconstructions, theta, phi_tilde, x_k_samps, log_z_given_y_phi, decoder_type):
    ...
    if decoder_type == 'standard':
        neg_reconstruction_error = expected_diagonal_gaussian_loglike(y, means_recon, out_2_recon, weights=r_nk)
    else:
    ...
    regualizer_term_final = torch.mean(regualizer_term_part_3)
...
def expected_diagonal_gaussian_loglike(y, param1_recon, param2_recon, weights=None):
    ...
    if weights is None:
        ...
        sample_mean = torch.sum(tf.pow(y.unsqueeze(1) - param1_recon, 2) / param2_recon) + torch.sum(tf.log(param2_recon))
        ...
        sample_mean = tf.einsum('nksd,nk->', torch.square(y - param1_recon)/ param2_recon + torch.log(param2_recon + 1e-8), weights)
